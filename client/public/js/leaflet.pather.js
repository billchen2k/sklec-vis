  ...
  L.Pather = L.FeatureGroup.extend({
    ...
    setMode: function setMode(mode) {
      ...
      /**
             ...
             */
      const shouldDisableDrag = function shouldDisableDrag() {
    ...
    setClassName: function setClassName(mode) {
      /**
             ...
             * @return {void}
             */
      const conditionallyAppendClassName = function conditionallyAppendClassName(modeName) {
  ...
  L.Pather.Polyline.prototype = {
    ...
    attachElbows: function attachElbows() {
      ...
      this.polyline._parts[0].forEach(function forEach(point) {
        const divIcon = new L.DivIcon({className: this.options.elbowClass});
        const latLng = this.map.layerPointToLatLng(point);
        const edge = new L.Marker(latLng, {icon: divIcon}).addTo(this.map);
    ...
    insertElbow: function insertElbow(latLng) {
      ...
      let insertAt = -1;
      const points = this.polyline._parts[0];

      points.forEach(function forEach(currentPoint, index) {
        const nextPoint = points[index + 1] || points[0];
        const distance = L.LineUtil.pointToSegmentDistance(newPoint, currentPoint, nextPoint);
      ...
      const parts = points.map(function map(point) {
        const latLng = this.map.layerPointToLatLng(point);
